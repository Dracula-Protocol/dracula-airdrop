...
const json_input_lp_contract = JSON.parse(fs.readFileSync('scripts/lp_on_contract_holders.json', { encoding: 'utf8' }));
if (typeof json_input_lp_contract !== 'object') {
  throw new Error('Invalid JSON');
}

console.log(" Total Contract LPs: ", json_input_lp_contract.length);

for (let item of json_input_lp_contract) {
  if (merged_data.has(item.address)) {
    let o : Format = merged_data.get(item.address)!;
    if (!(item.reasons in o.reasons)) {
      o.reasons = o.reasons.concat(item.reasons);
    }
    o.drc_held = o.drc_held.add(utils.parseUnits(item.amount.toString()).mul(DRC_PER_LP));
    merged_data.set(item.address, o);
...
for (let v of Array.from(merged_data.values())) {
  const share = v.drc_held.mul(HODLER_AMOUNT).div(total_drc_held);
